...
export class DialogAdapter extends EventEmitter {
  ...
  async getFirstOpenChannel() {
    ...
    if (this._myPublishClient < 0) return;
  ...
  async connect({ serverUrl, roomId, joinToken, serverParams, scene, clientId }) {
    ...
    if (this.enableAgoraRTC) clientConfig = { codec: "vp8", mode: "rtc" };
    else clientConfig = { codec: "vp8", mode: "live" };
  ...
  */
  ...
  async _manageSubscriptions() {
    ...
    if (
      AFRAME.scenes[0] &&
      this.limitSubscriptions &&
      (this._audioPubCount > this.maxAudioSubscriptions ||
        this._videoPubCount > this.maxVideoSubscriptions ||
        this.maxAudioDistanceApart > 0 ||
        this.maxVideoDistanceApart > 0)
    ) {
      ...
      if (this.enableVADControl) {
        ...
        for (var d = 0; d < distances.length && audioSubs < this.maxAudioSubscriptions; d++) {
          ...
          if (
            !expectedAudioSubs[peerId] &&
            this._audioPublishers[peerId] &&
            this._vadPublisherByPriority.indexOf(peerId) > -1 &&
            (this.maxAudioDistanceApart <= 0 || distances[d].distance < this.maxAudioDistanceApart)
          ) {
            audioSubs++;
            expectedAudioSubs[peerId] = peerId;
      ...
      for (var d = 0; d < distances.length && videoSubs < this.maxVideoSubscriptions; d++) {
        var peerId = distances[d].peerId;
        if (
          this._videoPublishers[peerId] &&
          !expectedVideoSubs[peerId] &&
          (this.maxVideoDistanceApart <= 0 || distances[d].distance < this.maxVideoDistanceApart)
        ) {
          videoSubs++;
          expectedVideoSubs[peerId] = peerId;
        }
      ...
      console.log(
        ...
        audioSubs,
        "audioExpect ",
        Object.keys(expectedAudioSubs).length,
        "audioPubs ",
        Object.keys(this._audioPublishers).length,
        "videoSubs ",
        videoSubs,
        "videoExpected ",
        Object.keys(expectedVideoSubs).length,
        "videoPubs ",
        Object.keys(this._videoPublishers).length,
        "audioExpected ",
        Object.keys(expectedAudioSubs)
      );
    } else {
  ...
  initRTM() {
    ...
    this.rtmClient
      .login({ token: null, uid: this.rtmUid })
      .then(() => {
  ...
  receiveRTM(senderId, text) {
    if (text.startsWith(this.VAD)) {
      ...
      while (this._vadPublisherByPriority.length > this.maxAudioSubscriptions * 2) {
        // keep more in case some leave
      ...
      while (this._vadPublisherRecentByPriority.length > this.maxAudioSubscriptions) {
        // keep more in case some leave
  ...
  sendVADEvent() {
    ...
    if (Date.now() - this.vadSend > this.vadSendWait) {
      ...
      this.rtmChannel
        .sendMessage({ text: this.VAD + ":" + this._clientId })
        .then(() => {
  ...
  sendNoVADEvent() {
    ...
    this.rtmChannel
      .sendMessage({ text: this.NoVAD + ":" + this._clientId })
      .then(() => {
  ...
  getInputLevel(track) {
    var analyser;
    if (
      track._source.volumeLevelAnalyser &&
      track._source.volumeLevelAnalyser.analyserNode &&
      track._source.volumeLevelAnalyser.analyserNode.frequencyBinCount
    ) {
  ...
  voiceActivityDetection() {
    if (!this.localTracks || !this.localTracks.audioTrack || !this.localTracks.audioTrack._enabled) return;
